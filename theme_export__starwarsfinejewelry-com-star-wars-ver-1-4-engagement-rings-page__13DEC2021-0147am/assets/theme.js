  ...
  Variants.prototype = Object.assign({}, Variants.prototype, {
    ...
    _onSelectChange: function() {
      ...
      if(!variant.available){
        $('.button-wrapper').hide();
        $('#mhaRnProduct').show();
        $('.pre-order-soldout').show();
        $('.pre-order-available').hide();
      }else{
        $('.button-wrapper').show();
        $('#mhaRnProduct').hide();
        $('.pre-order-available').show();
        $('.pre-order-soldout').hide();
      }
...
theme.MobileNav = (function() {
  ...
  $(document).on('click','button.mobile-nav__link',function(){
//   	$('.mobile-nav__dropdown').slideUp();
    var targetDiv = $(this).attr('data-target');
    $('[data-parent="'+targetDiv+'"]').slideToggle();
  });
...
theme.Product = (function() {
  ...
  Product.prototype = Object.assign({}, Product.prototype, {
    ...
    _addItemToCart: function(data) {
      ...
      var isCustomProductAdd = $('.product-form .name_engraving').attr('data-tag');
      if(isCustomProductAdd){
        var engravText = $('#embros_txt').val();
        if(engravText.length<1){
          $('.font-familt-grid input').prop('checked',false);
        }
...
//         .addClass(this.classes.cartPopupWrapperHidden);
    ...
    _switchMedia: function(mediaId) {
      ...
      var moveToMedia = $newMedia.data('media-id');
      $('.mm__product-gallery .slick-dots').find('button[data-media="'+ moveToMedia +'"]').trigger('click');
    },
      ...
      if (variant) {
        ...
        try{
          ...
          if (inventory_level == 0){
            $('.variant-inventory').css('color', '#E62900').html('Out of Stock!').show();          
          } else if (inventory_level > minStock){
            $('.variant-inventory').css('color', '#20B25B').html('In Stock!').show();
          } else {
            $('.variant-inventory').css('color', '#E62900').html('Only ' + inventory_level + ' left in Stock!').show();
          }
        }catch(e){
//           console.log(e);
        }
    ...
    $.get(recommendationsSectionUrl).then(
      function(section) {
        ...
        if (recommendationsMarkup.trim() !== '') {
          ...
          $('.sw__also-bought').slick({
            arrows: true,
            prevArrow:"<img class='slick-prev lazyload' data-src='https://cdn.shopify.com/s/files/1/0465/8787/1396/files/left-arrow-dark.svg?v=1599828644' data-sizes='auto'>",
            nextArrow:"<img class='slick-next lazyload' data-src='https://cdn.shopify.com/s/files/1/0465/8787/1396/files/right-arrow-dark.svg?v=1599828644' data-sizes='auto'>",
            infinite: false,
            autoplay: false,
            speed: 300,
            slidesToShow: 3,
...
$(document).ready(function() {
  ...
  $('.product-form .font-familt-grid input').on('change',function(){
    $('.font-error-msg').hide();
  })
  
  $(".buy-now-button .btn").on("click",function(e){
    e.preventDefault();
    var variantId = $('[name="id"]').find(":selected").val();
    var proQuantity = $('#Quantity-product-template').val();
    var engravText =  $(this).closest('.product-form').find('#embros_txt').val();
    var selectedFont = $(this).closest('.product-form').find('.font-familt-grid input:checked').val();
    console.log(selectedFont);
    if(engravText.length<1){
      $('.product-form .font-familt-grid').find('input').prop('checked',false);
    }else{
      if(!selectedFont){
        $('.font-error-msg').show().addClass('font-error-shake');
        setTimeout(function(){
          $('.font-error-msg').removeClass('font-error-shake');
        },600);
        return false;
      }
    }
    ...
    //add.js
    if(currentPageUrl.indexOf('/ja/')>-1){
      $.ajax({
        type: 'POST',
        url: '/cart/add.js',
        dataType: 'json',
        ...
        async: false,
        success: function(data){ 
          window.location.href="/ja/checkout";
        }
      });
    }else{
      $.ajax({
  ...
  // Hide wiser app if Japan language is active
  var websiteLanguage = window.location.href;
  if(websiteLanguage.indexOf('/ja/')>-1){
    $('body').addClass('no-evm-section');
  	$('.evm-also-bought,.evm-related-product,.evm-most-popular-products,.evm-related-viewed-browsing,.recently-viewed').hide().remove();
    $('body.no-evm-section .evm-also-bought').hide();
  }
  
  var videoElement = document.getElementById('home_video');
  var videoElementMobile = document.getElementById('home_mobile_video');
  var windowWidth = $(window).width();
  if(videoElementMobile != undefined && windowWidth<767){
    $('body').on('click touchstart', function () {
      if (videoElementMobile.playing) {
  ...
  if(videoElement != undefined && windowWidth>767){
    $('body').on('click touchstart', function () {
  ...
  //   $('.vid__container a').attr("href", link);
  //   var myCookie = Cookies.get('entered');
  //   if (myCookie != 'yes') {
  //     $('#myVideo').show();
  //   } else {
  //     $('#myVideo').hide();
  //   }
  //   $('.vid__container a').click(function(e) {
  //     e.preventDefault();
  //     // var setsession = window.sessionStorage.setItem('entered', 'yes');
  //     Cookies.set('entered', 'yes');
  //     $('#myVideo').hide();
  //   });
  
  var isHomepageTemplate = $('body').hasClass('template-index');
  console.log(isHomepageTemplate);
  ...
  var homePage = {
    ...
    initCollectionSlider: function() {
      var collectionSlider = $('.sw__collection-slider');
      if(collectionSlider.length) {
        collectionSlider.slick({
          dots: false,
          arrows: true,
          prevArrow:"<img class='slick-prev lazyload' data-src='https://cdn.shopify.com/s/files/1/0465/8787/1396/files/Left_Arrow.svg?v=1598777951'>",
          nextArrow:"<img class='slick-next lazyload' data-src='https://cdn.shopify.com/s/files/1/0465/8787/1396/files/Right_Arrow.svg?v=1598777951'>",
          infinite: false,
          speed: 500,
          slidesToShow: 3,
          slidesToScroll: 1,
    ...
    initImageSlider: function() {
      var imageSlider = $('.slide-content');
      if(imageSlider.length) {
        imageSlider.slick({
          dots: false,
          arrows: true,
          prevArrow:"<img class='slick-prev lazyload' data-src='https://cdn.shopify.com/s/files/1/0465/8787/1396/files/Left_Arrow_26fcb560-ed7c-4988-9ca3-90e70e5fb120.svg?v=1598796739'>",
          nextArrow:"<img class='slick-next lazyload' data-src='https://cdn.shopify.com/s/files/1/0465/8787/1396/files/Right_Arrow_29c5b4c8-924f-4a69-b2ce-072083d8a69b.svg?v=1598796739'>",
          infinite: false,
          speed: 500,
          slidesToShow: 1,
          slidesToScroll: 1,
    ...
    initMagnificPopup: function() {
      var popupButton = $('.craftsman-video a');      
    ...
    initInstagramSlider: function() {
      var collectionSlider = $('.sw__instaposts');
      if(collectionSlider.length) {
        collectionSlider.slick({
          dots: false,
          arrows: false,
    ...
    initCatSlider: function() {
      var catSlider = $('.sw__category_container');
      if(catSlider.length) {
        catSlider.slick({
          rows: 0,
          dots: true,
  ...
  var productDetailsPage = {
    initGallerySlider: function() {
      var galleryThumbnilSlider = $('.gallery-thumbnails');
      var galleryThumbnilCount = $('.gallery-thumbnails .image-wrapper').length;
      console.log(galleryThumbnilCount);
      if(galleryThumbnilSlider.length && galleryThumbnilCount > 5) {
        galleryThumbnilSlider.slick({
          vertical: true,
    ...
    shippingPopup: function() {
      $("#the-vault .close").click(function(e){
        $("#the-vault").css({"visibility":"hidden","opacity":"0"});
        Cookies.set('shipping-popup', 'open');
      });

      var popup = Cookies.get('shipping-popup');

      if(popup != "open"){
        $("#the-vault").css({"visibility":"visible","opacity":"1"});
      }
    },
    initNameEngraving: function(){
      // Code For custom tag Name engraving start
      var isCustomProduct = $('.product-form .name_engraving').attr('data-tag');
	  // console.log('data_tag', isCustomProduct);

      if (isCustomProduct != undefined){
        //changing metal color  
		// var color = $('.drop_color .custom-dropdown-selected').text().trim();
        // console.log('color', color);
        /*if (color == 'White Gold' || color == 'Silver'){
          $('#colr_img').attr('src','https://cdn.shopify.com/s/files/1/0465/8787/1396/files/White_Gold.jpg?v=1634620787');
        }
        if (color == 'Yellow Gold'){ 
          $('#colr_img').attr('src','https://cdn.shopify.com/s/files/1/0465/8787/1396/files/Yellow_Gold.jpg?v=1634620787');
        }
        if (color == 'Rose Gold'){ 
          $('#colr_img').attr('src','https://cdn.shopify.com/s/files/1/0465/8787/1396/files/Rose_Gold.jpg?v=1634620787');
        }*/

        /*$(document).on('click','.drop_color .custom-select-options',function(){
          var color = $('.drop_color .custom-dropdown-selected').text().trim();
          // console.log('color', color);
          if (color == 'White Gold' || color == 'Silver'){ 
            $('#colr_img').attr('src','https://cdn.shopify.com/s/files/1/0465/8787/1396/files/White_Gold.jpg?v=1634620787');
        ...
        function resize_to_fit() {
          var fontsize = $('#toChange p').css('font-size');
          console.log(fontsize,$('#toChange p').width(),$('#toChange').width());           
          if ($('#toChange p').width() >= $('#toChange').width()) {
            $('#toChange p').css('fontSize', parseFloat(fontsize) - 1);
            resize_to_fit();
          }
        }
        //changing font on name engraving start
        $('.name_engraving input[type="text"]').keyup(function(event) {
          var newText = event.target.value;
          $('#toChange p').html(newText);
          if(newText.length<1){
            $('#toChange p').html('Message');
            var fontLength = newText.length;
          }
          $('.product-single__media-wrapper').addClass('show360');
          $('.image_engraving').addClass('show360');
          
          if($(window).width()>767){
            $('#toChange p').css('font-size','15px');
          }else{
            $('#toChange p').css('font-size','12px');
          }
          resize_to_fit();
        });
        
        $('.name_engraving input[type="text"]').focus(function(event) { 
          var newText = event.target.value;
          $('#toChange p').html(newText);
          if(newText.length<1){
            $('#toChange p').html('Message');
        ...
        $('#english_font').click(function() {
          $('#toChange').css('font-family', 'Righteous');
        });
        
        $('#dancing_font').click(function() {
          $('#toChange').css('font-family', 'MandalorianFormal');
        });

        $('#tinos_reg').click(function() {
          $('#toChange').css('font-family', 'Aurebesh');
        }); 
        $('.font-family-btn').on('click',function(){
          if($(window).width()>767){
            $('#toChange p').css('font-size','15px');
          }else{
            $('#toChange p').css('font-size','12px');
        ...
        // Engraving image code css
        $('html,body').on('click touchstart','.gallery-thumbnails .image-wrapper a',function(){
          console.log('a');
          if($(this).hasClass('has-360view')){
            $('.gallery-thumbnails .image-wrapper a').removeClass('mz-thumb-selected');
          }
          var selectedZoom = $('.gallery-thumbnails .image-wrapper .mz-thumb-selected').length;
          if($(this).hasClass('has-360view')){
            $('.product-single__media-wrapper').addClass('show360');
            $('.image_engraving').addClass('show360');
          }else{
            $('.product-single__media-wrapper').removeClass('show360');
            $('.image_engraving').removeClass('show360');
          }
        })  
      }
      //changing font end
      // Code For custom tag Name engraving end
    }
  }
  
  var themePage = {
    initCatSlider: function() {
      var catSlider = $('.category_slider');
      var slideCount = parseInt($('.category_slider .starwars-category').length);
      var slideCountToShow = slideCount > 3 ? 3: slideCount;
      if(catSlider.length) {
        catSlider.slick({
          rows: 0,
          dots: false,
          arrows: true,
          prevArrow:"<img class='slick-prev lazyload' data-src='https://cdn.shopify.com/s/files/1/0465/8787/1396/files/Left_Arrow_26fcb560-ed7c-4988-9ca3-90e70e5fb120.svg?v=1598796739' data-sizes='auto'>",
          nextArrow:"<img class='slick-next lazyload' data-src='https://cdn.shopify.com/s/files/1/0465/8787/1396/files/Right_Arrow_29c5b4c8-924f-4a69-b2ce-072083d8a69b.svg?v=1598796739' data-sizes='auto'>",
          infinite: true,
          speed: 500,
    ...
    initOthCatSlider: function() {
      var catSlider = $('.cats_slider');
      if(catSlider.length) {
        catSlider.slick({
          rows: 0,
          dots: true,
  ...
  var kyberCrystalRadiance = {
    scrollVideo: function() {
      const html = document.documentElement;
      const scorllWarpper = document.getElementById("scroll-video-wrapper");
      const canvas = document.getElementById("hero-lightpass");
      const context = canvas.getContext("2d");

      const frameCount = 75;
      //       https://cdn.shopify.com/s/files/1/0420/2371/4967/files/WhatsApp_Video_2021-05-04_at_17.46.54_199.jpg?v=1620131367
      //       https://cdn.shopify.com/s/files/1/0465/8787/1396/files/Landing_Page_option_2_000.jpg?v=1621948427
      const currentFrame = index => (`https://cdn.shopify.com/s/files/1/0465/8787/1396/files/Landing_Page_option_2_${index.toString().padStart(3, '0')}.jpg`)
      ...
      imageComparesionScroll: function() {
        ...
        function drags(dragElement, resizeElement, container) {
          ...
          dragElement.on("mousedown touchstart", function(e) {
            ...
            dragElement.parents().on("mousemove touchmove", function(e) {
              ...
              $(".comparison-slider-wrapper .draggable").css("left", widthValue).on("mouseup touchend touchcancel", function() {
  ...
  var BlogPage = {
    initLatestBlogSlider: function() {
      var LatestBlogSlider = $('.blog_page .latest_blog_slider');
      if(LatestBlogSlider.length) {
        $(LatestBlogSlider).on('init', function(event, slick) {
          $(this).append('<div class="slick-counter small--hide"><span class="current"></span> / <span class="total"></span></div>');
          $('.current').text(slick.currentSlide<10?`0${slick.currentSlide+1}`:slick.currentSlide + 1);
        ...
        .slick({            
          ...
          dots: false,
          infinite: false,
          prevArrow:'<div class="prev-btn"><svg xmlns="http://www.w3.org/2000/svg" class="blog_arrow-prev" width="36" height="42" viewBox="0 0 36 42"><defs><style>.blog_arrow-prev .a{fill:#5e727d;}.blog_arrow-prev .b{fill:none;stroke:#fff;stroke-width:2px;}</style></defs><g transform="translate(-109 -2213.781)"><path class="a" d="M31.5,0,42,18,31.5,36h-21L0,18,10.5,0Z" transform="translate(109 2255.781) rotate(-90)"/><path class="b" d="M-6298.257,2306.837l-5.642,5.029,5.642,4.823" transform="translate(6428.398 -76.556)"/></g></svg></div>',
          nextArrow:'<div class="next-btn"><svg xmlns="http://www.w3.org/2000/svg" width="36" height="42" class="blog_arrow-next" viewBox="0 0 36 42"><defs><style>.blog_arrow-next .a{fill:#5e727d;}.blog_arrow-next .b{fill:none;stroke:#fafafb;stroke-width:2px;}</style></defs><g transform="translate(-156 -2213.781)"><path class="a" d="M31.5,0,42,18,31.5,36h-21L0,18,10.5,0Z" transform="translate(192 2213.781) rotate(90)"/><path class="b" d="M-6303.9,2306.837l5.642,5.029-5.642,4.823" transform="translate(6475.398 -76.556)"/></g></svg></div>',
          rows: 0,
          cssEase: 'ease-in-out',
          responsive:[
            {
              ...
              settings:{
                ...
                customPaging: function(slider, i) { 
                  return '<svg xmlns="http://www.w3.org/2000/svg" class="blog_slick_dots" width="16" height="13.538" viewBox="0 0 16 13.538"><defs><style>.blog_slick_dots .a{fill:#94a4ae;opacity:0.2;}.slick-active .blog_slick_dots .a{fill:#242424;opacity:1;}</style></defs><path class="a" d="M12,0l4,6.769-4,6.769H4L0,6.769,4,0Z"/></svg>';
                }
              }
            }
          ]
    ...
    initFeaturedBlog: function() {
      var collectionSlider = $('.featureBlog-slider');
      if(collectionSlider.length) {
        collectionSlider.slick({
          dots: false,
          arrows: true,
          prevArrow:'<div class="prev-btn"><svg xmlns="http://www.w3.org/2000/svg" width="36" height="42" viewBox="0 0 36 42"><defs><style>.a{fill:#5e727d;}.b{fill:none;stroke:#fff;stroke-width:2px;}</style></defs><g transform="translate(-109 -2213.781)"><path class="a" d="M31.5,0,42,18,31.5,36h-21L0,18,10.5,0Z" transform="translate(109 2255.781) rotate(-90)"/><path class="b" d="M-6298.257,2306.837l-5.642,5.029,5.642,4.823" transform="translate(6428.398 -76.556)"/></g></svg></div>',
          nextArrow:'<div class="next-btn"><svg xmlns="http://www.w3.org/2000/svg" width="36" height="42" viewBox="0 0 36 42"><defs><style>.a{fill:#5e727d;}.b{fill:none;stroke:#fafafb;stroke-width:2px;}</style></defs><g transform="translate(-156 -2213.781)"><path class="a" d="M31.5,0,42,18,31.5,36h-21L0,18,10.5,0Z" transform="translate(192 2213.781) rotate(90)"/><path class="b" d="M-6303.9,2306.837l5.642,5.029-5.642,4.823" transform="translate(6475.398 -76.556)"/></g></svg></div>',
          infinite: false,
          slidesToShow: 2.2,
          slidesToScroll: 1,
          rows:0,
  ...
  if(isHomepageTemplate){
    ...
    homePage.initCollectionSlider();
    homePage.initImageSlider();
    homePage.initMagnificPopup();
    homePage.initInstagramSlider();
    if (window.matchMedia('(max-width: 768px)').matches) {
      homePage.initCatSlider();
    }
  }

  if(websiteLanguage.indexOf('fathers-day-2021')>-1){
    homePage.initCollectionSlider();
  }
  ...
  storePopup.initMagnificPopup();
  
  productDetailsPage.initGallerySlider();
  productDetailsPage.shippingPopup();
  productDetailsPage.initNameEngraving();

  BlogPage.initLatestBlogSlider();
  BlogPage.initFeaturedBlog();

  themePage.initCatSlider();
  if (window.matchMedia('(max-width: 750px)').matches) {
    themePage.initOthCatSlider();
  }

  var isKyberTemplate = $('body').find('.page-kyber-crystal-radiance').length;
  if(isKyberTemplate>0){
    if (!window.matchMedia('(max-width: 750px)').matches) {
      kyberCrystalRadiance.scrollVideo();
    }
    kyberCrystalRadiance.imageComparesionScroll();
    homePage.initCollectionSlider();
  }

  if (window.matchMedia('(max-width: 768px)').matches) {
    // For Verical text top position in section
    var topVal = parseInt($('[data-section-type="starwars-newarrivals-section"] .vertical-text').width());
    $('[data-section-type="starwars-newarrivals-section"] .vertical-text').css('top', topVal + 25);
  }

  var limitedAlign = parseInt($('.limited_edition').width());
  $('.limited_edition').css('left', -limitedAlign + 40);

  // Customized quantity selector (jQuery)
  $(".product-quantity-box span").on("click",function(){
    var val = parseInt($("#Quantity-product-template").val());
    var self = $(this);
    if(self.hasClass('icon-plus')){
      val +=1;
    }else if(self.hasClass('icon-minus') && val > 0){
      val -=1;
    }
    if(val > 0){
      $("#Quantity-product-template").val(val);
    }
  });

  // PDP Tab + Accordion Script
  if (window.matchMedia('(min-width: 768px)').matches) {
    $('ul.tabs li').click(function(){
      var tab_id = $(this).attr('data-tab');
      $('ul.tabs li').removeClass('active');
      $('.tab-details .tab_content').removeClass('current');

      $(this).addClass('active');
      $("#"+tab_id).addClass('current');
    });
  } else if (window.matchMedia('(max-width: 768px)').matches) {  
    $('.tab__heading--small').on('click', function() {
      $('.tab__heading--small').not(this).removeClass('active');
      $('.tab__heading--small').not(this).next('.tab_content').slideUp();
      $(this).addClass('active');
  ...
  $('.learn_more').click(function() {
    $('html,body').animate({scrollTop: $(".about_content-section").offset().top},'slow');
  });

  $('.vd-collection-slider').slick({
    dots: false,
    arrows: false,
    infinite: false,
    speed: 300,
  ...
  // Footer accordian js
  if (window.matchMedia('(max-width: 768px)').matches) {
    $(document).on('click','.site-footer__item-inner .accordion_btn_foot',function(){
      $(this).toggleClass('active_foot');
      $(this).closest('.site-footer__item-inner').find('.site-footer__linklist').slideToggle();
    });
  }
...
// Register form validation
  function ValidateRegisterForm() {
    ...
    $('#CustomerRegisterForm input').removeClass('input--error');
    $('#CustomerRegisterForm [data-form-error]').html('').hide();
    title.closest('.input__group').removeClass('input--error');

    var emailPattern = /^[a-zA-Z0-9\-_]+(\.[a-zA-Z0-9\-_]+)*@[a-z0-9]+(\-[a-z0-9]+)*(\.[a-z0-9]+(\-[a-z0-9]+)*)*\.[a-z]{2,4}$/;
    
    var titelValue = title.val();
    //     alert(title.val());
    var StarWarURLJA = window.location.href;
    console.log('JA URL',StarWarURLJA);
    if(StarWarURLJA.indexOf('/ja/')>-1){
      if(titelValue.indexOf('タイトルを選択')>-1) {
        title.closest('.input__group')
      ...
      var FirstNameERR = '名を入力してください';
      var LastNameERR = 'あなたの姓を入力';
      var DOBERR = '生年月日を入力してください';
      var EmailERR = 'あなたのメールアドレスを入力してください';
      var EmailPatternERR = '有効なメールアドレスを入力してください';
      var PasswordERR = 'パスワードを入力してください';
      var PasswordLenERR = 'パスワードは6文字以上である必要があります';
      var ConfirmPassERR = '確認パスワードを入力してください';
      var ConfirmPassMatchERR = 'パスワードがパスワードと一致する必要があることを確認する';
    }else{
      ...
      var FirstNameERR = 'Please enter your first name';
      var LastNameERR = 'Please enter your last name';
      var DOBERR = 'Please enter your birth date';
      var EmailERR = 'Please enter your email';
      var EmailPatternERR = 'Please enter a valid email address';
      var PasswordERR = 'Please enter your password';
      var PasswordLenERR = 'Password must have more than 6 characters';
      var ConfirmPassERR = 'Please enter your confirm password';
      var ConfirmPassMatchERR = 'Confirm password should match with password';
    }
    ...
    if(StarWarURLJA.indexOf('/ja/')>-1){
      if(validation == true) {
        ...
        var firstNameString = firstNameValue.toString();
        if(firstNameString.indexOf('氏 ')<0 && firstNameString.indexOf('MS ')<0 && firstNameString.indexOf('Mrs ')<0){
      ...
      if(validation == true) {
        var firstNameValue = firstName.val();
        var firstNameString = firstNameValue.toString();
        if(firstNameString.indexOf('Mr ')<0 && firstNameString.indexOf('Ms ')<0 && firstNameString.indexOf('Mrs ')<0){
          firstName.val(titelValue+' '+firstNameValue);
        }
      }
    }
  ...
  $('#CustomerRegisterForm .btn--primary').on('click', function(e) {
...
//   $(".evm-ws__cart-submit").click(function(){
  $(document).on('click','.evm-ws__cart-submit',function(){
   ...
   $(this).addClass("evm-loader-view"); 
   var curevl=$(this); 
     setTimeout(function(){
        curevl.html('<span>ADD TO BAG</span>');
        curevl.removeClass("evm-loader-view");
       $.getJSON('/cart.js').then(
         function(cart) {
...
$(document).ready(function (){
  $('.er-main-banner .banner-btn').on('click', function (e){
    e.preventDefault();
    $('html, body').animate({
      scrollTop: $('.er-custom-section-1').offset().top - 30
    }, 2000);
  });
});